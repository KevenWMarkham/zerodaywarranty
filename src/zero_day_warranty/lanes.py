...
<head>
  ...
  * { box-sizing: border-box; }
  ...
  .d3btn { display: inline-flex; align-items: center; gap: 7px; font-family: Aptos, sans-serif; font-size: 13px; font-weight: 700; padding: 9px 16px; border-radius: 8px; border: none; background: linear-gradient(135deg, var(--navy), var(--ms-blue-dark)); color: #fff; text-decoration: none; box-shadow: 0 2px 8px rgba(0,0,0,0.12); }
  .d3btn:hover { background: linear-gradient(135deg, var(--ms-blue-dark), var(--ms-blue)); }
  .navbtn { display: inline-flex; align-items: center; gap: 7px; font-family: Aptos, sans-serif; font-size: 13px; font-weight: 700; padding: 9px 16px; border-radius: 8px; border: 1px solid var(--ms-blue); background: #fff; color: var(--ms-blue-dark); text-decoration: none; }
  .navbtn:hover { background: #EAF3FB; }
  .btnrow { display: flex; flex-wrap: wrap; gap: 10px; margin: 0 0 18px; }
  .d3link { font-family: Aptos, sans-serif; font-size: 11.5px; font-weight: 700; color: var(--ms-blue-dark); text-decoration: none; }
  .d3link:hover { text-decoration: underline; }
  ...
  <div class="portal">
    ...
    <div class="portal-body">
      ...
      <div class="panels">
        ...
        <div class="panel span2">
          <div class="panel-title">Lane handoff <span class="meta">where it sits in the chain</span>
            <a class="d3link" href="{_PROCESS_3D}#lane={view.id}">▶ view this lane in 3D</a></div>
          <div class="handoff">
...
<header class="cover">
  ...
  <div class="meta">
    ...
    <span><strong>Companion to:</strong> <a href="{_SWIMLANES}">Capability Swim Lanes</a> · <a href="{_PERSONA_PORTALS}">Persona Portals</a> · <a href="{_ARCHITECTURE}">Architecture</a> · <a href="{_AZURE_DEPLOY}">Azure Deployment</a></span>
  </div>
</header>"""
  ...
  function activateTab(id, scroll) {
    ...
    var panel = document.querySelector('.tab-panel[data-panel="' + id + '"]');
    if (!btn || !panel) return;
  ...
  // Deep-link: ...#lane=<id> (or #<id>) opens that lane's tab (e.g. from the 3D view).
  function tabFromHash() {
    var m = location.hash.match(/lane=([a-z0-9-]+)/i) || location.hash.match(/^#([a-z0-9-]+)/i);
    if (m) activateTab(m[1], false);
  }
  tabFromHash();
