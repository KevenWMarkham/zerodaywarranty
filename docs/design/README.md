...
| [`ZeroDayWarranty_Azure_Deployment.html`](ZeroDayWarranty_Azure_Deployment.html) | **Azure deployment design** — **self-contained** in resource group `Agentic-Automotives` (<subscription-name> subscription); every resource created fresh, nothing shared. Topology, resources created, identity & RBAC, medallion-on-Postgres, model mapping, secrets, deploy sequence. Reference Bicep in [`../../infra/`](../../infra/). |
